<?php 
	...
	$strAccion = (isset($_POST['accion'])) ? $_POST['accion'] : 'abrirPersonajeImagenes';

	// dependiendo de la acción de usuario se solicita una acción u otra al controlador
	switch ($strAccion) {	


		// se ha solicitado la acción subirPersonajeImagen
		case 'subirPersonajeImagen':
			$cp->subirPersonajeImagen();
			break;	


		// se ha solicitado la acción eliminarPersonajeImagen
		case 'eliminarPersonajeImagen':
			$cp->eliminarPersonajeImagen();
			break;	


		// si no hay acción se elige la acción por defecto que es obtener la lista de imágenes
		// del personaje
		default:
			$cp->abrirPersonajeImagenes();
			break;
	}
