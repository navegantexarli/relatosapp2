<?php 
	...
	// se incluye el controlador de relaciones
	include_once ("controlador/ControlRelaciones.php");
	...
	// se inicializa el controlador de relaciones
	$cr = new ControlRelaciones();
	...
	$strAccion = (isset($_POST['accion'])) ? $_POST['accion'] : 'abrirRelaciones';

	// dependiendo de la acción de usuario se solicita una acción u otra al controlador
	switch ($strAccion) {


		// se ha solicitado la acción buscarRelaciones
		case 'buscarRelaciones':
			$cr->buscarRelaciones();
			break;


		// se ha solicitado la acción crearRelacion
		case 'crearRelacion':
			$cr->crearRelacion();
			break;


		// se ha solicitado la acción actualizarRelacion
		case 'actualizarRelacion':
			$cr->actualizarRelacion();
			break;			


		// se ha solicitado la acción eliminarRelacion
		case 'eliminarRelacion':
			$cr->eliminarRelacion();
			break;			


		// si no hay acción se elige la acción por defecto que es obtener la lista de relaciones
		default:
			// se solicita abrir la ventana de relaciones
			$cr->abrirRelaciones();
			break;
	}
